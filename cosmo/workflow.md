...
**Project directory convention:** Every project lives in its own named subfolder inside `cosmo-workspace/` with its own git repo:
```
cosmo-workspace/
  cosmo/          ← cosmo framework (tracked by cosmo-workspace git)
...
1. Create the project folder: `mkdir cosmo-workspace/PROJECT_NAME/`
...
5. Never scaffold at the `cosmo-workspace/` root — that git only tracks `cosmo/`
