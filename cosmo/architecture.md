...
**Why:** Prevents circular dependencies, makes features independently testable, allows features to be extracted/replaced easily.

**Check command:** `grep -r "from ['\"].*features/" src/features/`
...
3. Respect import boundaries (see check command above)
4. Centralize any new configuration
5. Verify Definition of Done before proceeding to review

## When Reviewing (Phase 4)

Check:
1. **Run import boundary check** (see command in Import Boundaries section)
